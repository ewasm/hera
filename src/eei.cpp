  ...
  Literal EthereumInterface::callImport(Import *import, LiteralList& arguments) {
    ...
    if (
      ...
      if (import->base == Name("call"))
        ...
        kind = EEI_CALLDELEGATE;
      else if (import->base == Name("callStatic"))
        kind = EEI_CALLSTATIC;
      else
        heraAssert(false, "");

      if ((kind == EEI_CALL) || (kind == EEI_CALLCODE)) {
      ...
      if (kind == EEI_CALL || kind == EEI_CALLCODE) {
        ...
        call_message.kind = (kind == EEI_CALLCODE) ? EVMC_CALLCODE : EVMC_CALL;

        if ((kind == EEI_CALL) && !isZeroUint256(call_message.value)) {
      ...
      if (!isZeroUint256(call_message.value)) {
        ...
        if ((kind == EEI_CALL) && !context->fn_table->account_exists(context, &call_message.destination))
